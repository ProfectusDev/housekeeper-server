...
// Retrieve a list of criteria linked to house_id
app.post('/api/getCriteria', function(req, res) {
  var house_id = req.body['hid'];
  var query_str = "SELECT * FROM Criteria WHERE (hid = '" + house_id + "');";
  connection.query(query_str, function(error, results, fields) {
    if (error) {
      res.status(500).send('Error: ' + error.code);
      console.log('Error: ' + error.code);
    } else {
      res.send('Successfully Retrieved Criteria');
      console.log('Retrieved Criteria from table: ' + house_id);
    }
  });
})
